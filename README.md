...
连接 fd —— 那是 socket 匿名 inode）。两者不一致说明**连上了，但对面不是你以为
的那个端点** —— 挂载视图分叉、端点路径错配、多实例串台都属于这类，而它们原本
会一路静默到出错帧。现在客户端报错拒绝连接（本仓库两个客户端实现均如此），
而不是带病继续跑。

> 注意区分：平台把单个 socket 文件而非目录 bind mount 时，daemon 重启换 inode
> 后容器侧持死引用 —— 那一类**在 `connect()` 阶段就被内核 `ECONNREFUSED` 拦下**
> （真机实测），走不到握手，由连接错误兜住，不归 inode 校验管。目录级挂载是正解。

客户端还必须容忍旧 daemon：v0.3.2 及更早按严格相等判版本，见到 v3 会回
`status=1` 并断开。本仓库客户端在此情况下自动重连降级 v2 再试（inode 校验
随之跳过并打印说明）—— daemon 由平台投放、驱动独立更新，错配是常态。
