...
**吞吐现状（诚实记录）**：该流实测 **48 fps < 60 fps**。两遍法是每个像素帧
额外再发一遍"DPB 修复帧"，硬件实际解码约 3300 次，这就是那 20% 的缺口。
本地播放器可以靠预解码缓冲掩盖，浏览器逐帧实时播则有掉帧风险。
真正的解法是单遍化 —— 需要 ffmpeg 侧把 `ref_frames[]` / 当前显示帧的
surface 身份交给驱动，从而省掉修复趟，尚未开工。
