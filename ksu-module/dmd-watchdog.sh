...
while : ; do
    ...
    1|2|8)
        # 1 = 连不上（没在跑）
        # 2 = 连上了但握手失败（僵死/半死）
        # 8 = 握手成功但零帧回传 —— 与 2 同性质：进程活着却不服务。
        #     这一类是旧探针（只验握手）会误判成 healthy 的盲区：真机上
        #     Unix socket 缓冲不足导致吞吐塌陷时，daemon 能握手、能收 NALU，
        #     却一帧解不出来，容器内浏览器静默回落 CPU 软解，而看护全程报
        #     healthy 不做任何处置。探针改为真码流出帧判据后才暴露出来。
        if [ "${rc}" = "2" ] || [ "${rc}" = "8" ]; then
            if [ "${rc}" = "8" ]; then
                log "daemon 握手成功但零帧回传 —— 解码链路已失效，需要重启"
            else
