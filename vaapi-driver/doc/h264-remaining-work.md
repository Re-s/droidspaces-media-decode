# H.264 / HEVC 解码路径：实现前的分析记录（大部分已完成）

> ⚠️ **本文写在实现之前，标题里的"没做"已经不成立。**
>
> **当前状态：H.264 与 HEVC 都已完整实现并通过逐字节验证。**
> H.264 六条流 + 长流 3000 帧、HEVC 五条流（含 4K、长流 1500 帧）均与
> 软解基线逐字节一致；浏览器 H.264 736 帧 0 黑帧 30.7 fps、
> HEVC 697 帧 30.3 fps。下文"目前返回 UNIMPLEMENTED"之类的陈述已过期。
>
> 保留本文是因为其中的**码流分析与踩坑记录仍然准确且有价值**
> （slice 载荷的真实构成、参数集字段推导、emulation prevention 处理等）。
> 读它时请区分：
>
> - **仍然有效**：对 VA-API 传入数据的分析、字段语义、合成方法
> - **已完成**：清单里约 13 项待办已实现
> - **仍待办（2 项）**：seek/flush 路径的端到端验证、丢尾部帧的改善
> - **已被推翻的建议（3 条，勿照做）**：
>   1. 「PPS 默认值改用 `num_ref_frames - 1`」—— 实施过，是**回归**：
>      `long3000.h264` 只解出 15/1323 帧。真因是参数集重送在承担纠错作用
>      （正常版发 4 种变体共 2735 次，含真值），收敛成单一份就赌错了。
>      当前实现直接用 slice param 的生效值。
>   2. 「按 POC 配对，不按提交顺序」—— 已被更好的方案取代：daemon 回传
>      输入单元序号（`CAP_FRAME_PTS`），驱动按序号精确匹配，与输出顺序
>      完全解耦。POC 重排只作为旧 daemon 的回退路径。
>   3. 「不要改 daemon」—— 已经改了，而且是关键改动：加
>      `vendor.qti-ext-dec-picture-order.enable=1`（输出滞后 4→1）
>      与每帧回传输入单元序号。
>
> 权威的当前状态见仓库根 `CHANGELOG.md` 与 `vaapi-driver/README.md`。

本文记录（实现前）停下来的真实原因与已核实的事实。
