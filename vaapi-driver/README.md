...
> ⚠️ 这里曾写着"功能正确，只是慢"，依据是 `tools/probe_rebuild.c`。
> **那个依据不成立** —— 该探针只数帧数没看画面（帧全是纯黑 Y=16），
> 文件开头已自我标注结论不可信。
>
> 重建的真实代价是**参考帧链被摧毁**：续传后要一直黑到下一个 IDR。
> 不是"慢"，是"画面坏"。这正是浏览器"画面一闪一闪"的根因
> （实测 135/708 帧纯黑）。当前实现的对策是把触发条件收紧到几乎不发生。
