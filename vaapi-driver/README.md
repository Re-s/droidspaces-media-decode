...
| 驱动侧 / 浏览器侧错误 | 无（但见下方画面缺陷） |
| 实际帧率 | **29.8 fps**（每帧 33.6 ms，视频标称 30fps） |
| 画面正确性 | ⚠️ **黑屏闪烁**，约 9 成帧为纯黑，见上文"已知缺陷" |

⚠️ 注意"零错误 + 满帧率"**不等于画面正确**。这套指标全绿的同时画面是坏的 ——
错误日志、帧计数、解码耗时都发现不了参考帧链被摧毁。
判断解码正确性必须检查像素（`tools/probe_black.c`）。
...
⚠️ **已知缺陷：画面会黑屏闪烁。** 两种手段都会摧毁参考帧链 ——
`flush` 和重建会话在这一点上**没有区别**，实测 60 帧样本里各有 54 帧纯黑
（`tools/probe_black.c`，亮度均值恰为 16 即 BT.601 black level）。
H.264 的 P/B 帧必须依赖参考帧，从非 IDR 位置重新开始解码要一直黑到
下一个 IDR（本测试流每 30 帧一个，故最多连黑 29 帧）。

这个缺陷**不是可逆排空引入的**：把 daemon 回退到无排空支持的版本
（驱动自动退回 `finish_input` + 重建）重测，黑帧同样是 54。
它从"实现会话重建"起就存在，只是此前从未检查过像素内容。

解法已验证但**尚未实现**（`tools/probe_replay.c`）：排空后不能从当前位置
续传，必须从最近的 IDR 重放并丢弃重放产生的帧 —— 那样黑帧与重复帧都是 0。
代价是要在驱动里缓存整个 GOP，且重放开销会拉低帧率，需重新测量。

更根本的方向是让解码器在浅队列下就出帧，从而完全不需要排空。
`tools/probe_keys.c` 逐键测过（滞后目标 <= 3）：
...
只有最后一个有效。但它**不能直接用**：开了之后解码器按解码顺序输出，
而驱动的 POC 配对假设的是显示序，实测 `test1080.h264` 帧数正确（150）
但 105 帧错位（每 4 帧错 3 帧）。已撤销。

要用它必须同时把配对逻辑改成按解码序 FIFO 并自行承担重排 ——
那是独立改动，收益是彻底消除黑帧，值得单独评估。
