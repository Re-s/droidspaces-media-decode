...
**帧头必须把 PTS 字段读掉。** daemon 是**无条件**发送该字段的：内联路径帧头
共 4 个字（16 字节）`[宽][高][长度][PTS]`，SHM 路径控制消息共 6 个字（24 字节）
`[宽][高][SHM哨兵][槽位][长度][PTS]`。少读这一个字会让下一帧从 PTS 开始解析，
整条流从此错位。

注意格式描述块里的 `CAP_FRAME_PTS` 能力位只是**告知**客户端该字段存在，
**不是发送开关** —— 把它当开关理解曾导致本参考实现两条路径都漏读该字段。
`DecodedFrame.unit_seq` 承载这个值（该帧对应的输入单元序号），
可用于把输出帧与送入的输入单元精确配对。
...
每处理完一帧必须调 `comm_release_frame` 归还槽位。池耗尽时 daemon 会**等待**
（正常背压，不是故障），最多等 `SHM_SLOT_WAIT_MS`（15 秒）才判定客户端卡死并结束会话。
两种模式导出的 PPM 逐字节一致。daemon 可能降级为 TCP，用 `comm_get_xfer` 查实际模式。

> 吞吐数字请以 `doc/performance-and-roadmap.md` 为准。此处曾写
> 「166.8 → 194.7 fps，daemon CPU 降 28.6%」，那组数字来自独立测试程序
> `tests/test_dmd_client.c`，不是驱动被 dlopen 进真实消费者进程的结果；
> 真实环境实测是 daemon 进程级 CPU 降约 19%，且**不代表系统级 CPU
> 或整机功耗下降**（系统级实测硬解 57.70 vs 软解 57.40 ms/帧，差 0.5% 在噪声内）。
