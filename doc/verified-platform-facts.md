...
0.4.0 改为驱动内 V4L2 直通后，本文各节的时效差别很大。逐节核对如下
（"仍成立"的部分在第 93 轮于本机重新复核过）：

| 节 | 0.4.0 下的状态 |
|---|---|
| §1 跨边界通信的三条通道 | ❌ **整节不适用**（230 行）。0.4.0 没有跨边界通信：驱动在容器进程内直接 `open("/dev/video32")`。Unix socket / TCP / abstract socket 的对比、netns 归属对通道的影响、SELinux binder 规则全部与当前架构无关。其中的 namespace 实测数据本身准确，仅"通道选择"这个用途消失了 |
| §2 端到端解码链路已验证可用 | ⚠️ 结论仍成立但**取证链路已变**：当年经 daemon，现在经 V4L2 |
| §3 输出分辨率是 1920x1088 | ✅ **仍成立**，与架构无关（Venus 的对齐规范）。本机复核：`G_FMT(CAPTURE)` 默认返回 1920x1088。⚠️ 0.4.3 补充了关键的一半：那个 1920x1088 是**打开设备时的默认值，不随 `S_FMT(OUTPUT)` 联动**，见 §3.1 |
| §4 输入 NALU 数与输出帧数不是 1:1 | ✅ **仍成立**，是解码器行为而非传输行为 |
| §5 设备硬件解码能力清单 | ✅ **仍成立**（来自 `media_codecs.xml` 与 `ENUM_FMT`） |
| §6 容器内 V4L2 节点状态 | ✅ **仍成立且现在是核心**。本机复核节点表一致：video32=`msm_vidc_vdec`、video33=`msm_vidc_venc`、video34 `QUERYCAP` 返回 `EINVAL` |
| §6.5 `msm_vidc` 中断计数可作硬解证据 | ✅ **仍成立，且本轮系列反复用到**。第 85~87 轮正是靠它区分"固件在跑"与"固件没动"（编码器 753 IRQ vs 解码器 ~0） |
| §7 服务端稳定性 | ❌ 不适用，没有服务端了 |
| §8 构建 | ⚠️ 部分不适用：`build.sh` 与交叉编译 daemon 的部分已删除；驱动的构建见 `vaapi-driver/Makefile` |
| §9 能效实测 | ⚠️ 数字属旧架构，但**方法论与结论方向仍有参考价值**（单路硬解不省电，价值在并发）。V4L2 路径的能效尚未测量 |
...
> `droidspaces-gpu` 组权限即可工作（实测），不需要 root。
...
## 3.1 `G_FMT(CAPTURE)` 的 1920x1088 是残留默认值，不随 OUTPUT 联动

验证日期：2026-09-03（0.4.3 绿屏排查）。这条是 §3 缺掉的另一半，
也是浏览器绿屏的直接根因。

`G_FMT(CAPTURE)` **总是**返回 msm_vidc 打开设备时的默认几何 `1920x1088`，
**不随 `S_FMT(OUTPUT)` 联动** —— 即使 OUTPUT 已协商成 1280x720，
CAPTURE 侧读回来的仍是 1080p 那一套。

驱动会用 OUTPUT 的协商值覆盖宽高，但**驱动不回填 `bytesperline` 与
`sizeimage`**，这两个字段仍是 1080p 的值。于是同一个 `v4l2_format`
里出现了自相矛盾的几何：宽高是 720p 的，行距与缓冲大小是 1080p 的。

**只检查下限的保护会漏掉它。** 实测 1280x720（前一条流是 1080p）：

| 字段 | 读回值 | 真实应为 |
|---|---|---|
| width x height | 1920x1088（被我们改写成 1280x720） | 1280x720 |
| `bytesperline` | **1920** | 1280 |
| `sizeimage` | 1080p 的值 | 1280×736×3/2 |

`stride < w` 这类下限检查在这里不成立（`1920 < 1280` 为假），错误 stride
被原样保留，造成双重损坏：

1. 按 `stride=1920` 取 1280 宽的帧，**每行错位 640 字节** —— 表现为斜纹、
   色块、绿屏；
2. 用 `cap_size` 反推 slice_height 得 **492**（真实 736），于是每帧截断
   —— 日志里的 `帧需 1416960 字节 > dumb buffer 1413120 字节`。
...
| 维度 | 对齐 | 实测取值 |
|---|---|---|
| CAPTURE stride | **128 字节** | 见下表 |
| CAPTURE 高度（slice_height） | **32 行** | 1080→1088、720→736、2160→2176、480→480、360→384 |

因此合法 stride 必落在 **`[align(w,128), align(w,128)+128)`** 区间内，
超出这个区间就是残留值，可按对齐规则直接重算。这比"和宽度比大小"可靠，
因为它同时给出上下界。

### 各分辨率纠正后的实测几何

| 显示分辨率 | stride | slice_height | 备注 |
|---|---|---|---|
| 1920x1080 | 1920 | 1088 | 不需纠正（恰好等于默认几何，所以旧回归测不出问题） |
| 1280x720 | 1280 | 736 | |
| 854x480 | **896** | 480 | 宽 854 非 128 倍数；实际编码尺寸被对齐成 **856** |
| 640x360 | 640 | 384 | |
| 720x1280 | **768** | 1280 | 竖屏 |

854x480 那一行是最容易算错的：显示宽 854 既不是 128 的倍数，编码尺寸本身
也已被对齐到 856，而 stride 又要在 856 之上再对齐到 896。**三个宽度值都不
相等**，任何一处混用都会错位。

> **验证方法**：不能只看"能解出来"。这几组几何全部以**硬解与软解逐字节
> 比对（md5 一致）**为判据 —— 错位 640 字节的帧照样能解完、帧数照样对，
> 只有逐字节比对才抓得住。
...
## 10. NV12 里 `UV=0` 是最大色偏，不是无色

验证日期：2026-09-03。这条不是设备特性而是色彩空间的数学性质，
但它在本项目里反复出现，值得当作事实档案的一条。

NV12 的 UV 平面填 0 **不等于没有颜色**。经限制范围 BT.601 转 RGB 得到：

```
R ≈ 0, G ≈ 135, B ≈ 0      →  纯绿 rgb(0,135,0)
```

**无色偏的中性值是 128**，不是 0。所以：

| 缓冲内容 | 屏幕表现 |
|---|---|
| Y=0, UV=0（整块 `memset` 清零） | **纯绿** |
| Y=0, UV=128 | 纯黑 |

**这条事实的用处**：它是一个指纹，能区分两类完全不同的故障。

| 屏幕是 | 说明 |
|---|---|
| **纯绿** | 缓冲还是那份全零初始值 —— **数据从未到达**（时序问题、导出早于解码、会话没起来） |
| 花屏 / 斜纹 / 局部色块 | 数据到了但**几何算错**（stride / slice_height 错位，见 §3.1） |

把这两者混为一谈会把排查带偏一整轮：前者要查时序与数据通路，
后者要查几何。

对应的实测判据（`DMD_VA_LUMA` 探针统计导出时刻的 UV 平面）：
...
判定纯绿的门限取 **UV 近零比例 > 80%**。用比例而不是均值，
是因为局部色块场景下均值会被正常区域拉回来。

## 11. dumb buffer 的 cache 维护：`msync` 不可用，`DMA_BUF_IOCTL_SYNC` 才是接口

验证日期：2026-09-03。

### 11.1 `msync` 对 dumb buffer 返回 `EINVAL`

DRM dumb buffer 的 mmap 是 **`VM_PFNMAP` / `VM_IO`** 映射，**没有页缓存可回写**，
所以 `msync` 在它上面无意义 —— 本机实测直接返回 `EINVAL`。

正确接口是 `DMA_BUF_IOCTL_SYNC`。且 `linux/dma-buf.h` 明确要求
**START / END 成对**使用：
...
**只调 END 不保证触发 clean。** 本项目在这里走过两步弯路：先是完全没做同步，
后来只补了一个孤立的 END —— 两者都不符合规范。现在 CPU 对 dumb buffer 的
写入被成对包裹。

### 11.2 被推翻的假设：cache 未刷新**不是**绿屏的原因

排查中长期怀疑是 CPU cache 未刷到 GPU（`memcpy` 的数据停在 D-cache，
GPU 零拷贝读到旧数据）。**该假设已被实测推翻。**

取证方式（关键是在**真实解码流程**上做对照，不是合成测试）：
用 `LD_PRELOAD` 把导出的 dma-buf fd 按外部消费方那样重新 `mmap`，
再与 `vaDeriveImage` 的 CPU 路径读到的字节逐一比对：
...
**结论：数据确实在被导出的那块内存里**，逐字节相同。绿屏与 cache 无关，
真因是 §3.1 的几何。§11.1 的成对 sync 仍然保留（它符合规范），
但**不要**把它当成绿屏的修复项。

### 11.3 对照参考：Mesa 的 VA 驱动根本不做这件事

本机自带的两个 Mesa VA 驱动里，相关字样出现次数：

| 驱动 | `DMA_BUF_IOCTL_SYNC` | `PRIME_HANDLE_TO_FD` |
|---|---|---|
| `radeonsi_drv_video.so` | **0** | **0** |
| `virtio_gpu_drv_video.so` | **0** | **0** |

它们走的是 `pipe_resource` + `PIPE_BIND_*`，即**用 GPU 自己的分配器**分配
可导出内存，因此不需要「dumb buffer + CPU memcpy + 手工 cache 维护」这一套。

**本项目为什么不同**：拿不到 MediaCodec / V4L2 缓冲的 fd，只能自己分配可导出
