...
`cfg.want_shm = 0`，注释说明"阶段 1 固定 TCP：SHM 是后续优化，
先把正确性做对"。这是全仓库唯一一处赋值，所以：

- 驱动**从不主动请求** SHM，浏览器路径从未走过它
- 它在 Firefox 沙箱下能否工作**尚未验证**
- 要启用需改这一行（或改成读环境变量）

⚠️ 读代码时容易误判：这段功能完整、注释详尽，看着像"在用"，
实际从未被执行。判断某能力是否生效，要先确认它的开关是开的。
